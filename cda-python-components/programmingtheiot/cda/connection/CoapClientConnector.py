...
class HandleActuatorEvent:
    """
    Helper class to handle CoAP observe responses for actuator resources.
    """
    ...
    def __init__(self,
        ...
        self.resource = resource or ResourceNameEnum.CDA_ACTUATOR_CMD_RESOURCE
...
class CoapClientConnector(IRequestResponseClient):
    """
    CoAP Client implementation utilizing the CoAPthon3 library.
    """
    ...
    def sendDiscoveryRequest(self, timeout: int = IRequestResponseClient.DEFAULT_TIMEOUT) -> bool:
        ...
        try:
            ...
            logging.error(f"Failed to send DISCOVERY request: {str(e)}")
            traceback.print_exception(type(e), e, e.__traceback__)
            return False
    ...
    def sendDeleteRequest(self, resource: ResourceNameEnum = None, name: str = None,
        ...
        if resource or name:
            ...
            try:
                ...
                logging.error(f"Failed to send DELETE request to {resourcePath}: {str(e)}")
                traceback.print_exception(type(e), e, e.__traceback__)
                return False
    ...
    def sendGetRequest(self, resource: ResourceNameEnum = None, name: str = None,
        ...
        if resource or name:
            ...
            try:
                ...
                response = self.coapClient.send_request(
                    request = request,
                    timeout = timeout
                )
                ...
                logging.error(f"Failed to send GET request to {resourcePath}: {str(e)}")
                traceback.print_exception(type(e), e, e.__traceback__)
                return False
    ...
    def sendPostRequest(self, resource: ResourceNameEnum = None, name: str = None,
        ...
        if resource or name:
            ...
            try:
                ...
                logging.error(f"Failed to send POST request to {resourcePath}: {str(e)}")
                traceback.print_exception(type(e), e, e.__traceback__)
                return False
    ...
    def sendPutRequest(self, resource: ResourceNameEnum = None, name: str = None,
        ...
        if resource or name:
            ...
            try:
                ...
                logging.error(f"Failed to send PUT request to {resourcePath}: {str(e)}")
                traceback.print_exception(type(e), e, e.__traceback__)
                return False
    ...
    def startObserver(self, resource: ResourceNameEnum = None, name: str = None,
        ...
        if resource or name:
            ...
            observeHandler = HandleActuatorEvent(
            ...
            try:
                self.coapClient.observe(
                    path = resourcePath,
                    callback = observeHandler.handleActuatorResponse
                )

                logging.info(f"Started observing resource {resource} at path: {resourcePath}")
                return True
        ...
        logging.warning("No resource or name provided for startObserver.")
        return False
    ...
    def stopObserver(self, resource: ResourceNameEnum = None, name: str = None,
        ...
        logging.warning("No resource or name provided for stopObserver.")
        return False
        ...
        if resourcePath:
            ...
            if len(locationPath) > 2:
                ...
                if dataType == ConfigConst.ACTUATOR_CMD:
                    logging.info(f"ActuatorData payload received: {jsonData}")

                    try:
                        ad = DataUtil().jsonToActuatorData(jsonData = jsonData)

                        if self.dataMsgListener:
                            self.dataMsgListener.handleActuatorCommandMessage(ad)

                    except Exception as e:
                        logging.warning(f"Failed to decode actuator data. Ignoring: {jsonData}. Error: {str(e)}")
                        traceback.print_exception(type(e), e, e.__traceback__)
