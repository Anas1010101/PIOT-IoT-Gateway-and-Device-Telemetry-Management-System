...
class MqttClientPerformanceTest(unittest.TestCase):
	"""
	This test case class contains performance tests for
	MqttClientConnector.

	It measures:
	- Connect / Disconnect performance
	- Publish performance using QoS 0, 1 and 2
	"""
	...
	def _execTestPublish(self, maxTestRuns: int, qos: int):
		...
		payload = DataUtil().sensorDataToJson(data=sensorData)

		startTime = time.time_ns()

		for seqNo in range(maxTestRuns):
