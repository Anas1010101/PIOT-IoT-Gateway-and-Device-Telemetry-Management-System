...
class MqttClientConnectorTest(unittest.TestCase):
	...
	def setUp(self):
		# Different client ID from the CDA to avoid MQTT broker conflicts.
		self.mcc = MqttClientConnector(clientID = "constraineddevice001_Test")
	...
	def testCDAManagementStatusSubscribe(self):
		...
		self.mcc.subscribeToTopic(
			resource = ResourceNameEnum.CDA_MGMT_STATUS_CMD_RESOURCE,
			qos = qos
		)
